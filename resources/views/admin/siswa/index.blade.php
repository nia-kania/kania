...
<body>
  ...
  <a href="{{ route('logout') }}" onclik="event.preventDefault(); documen.getElementById('logout-form').submit();">logout</a>
  <br><br>
...
<tr>
  ...
  <td>
    <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('siswa.destroy', $siswa->id) }}" method="POST">
      ...
      <a href ="{{ route('siswa.edit', $siswa->id) }}" class="btn-sm btn-primary">EDIT</a>
      @csrf
      @method('DELETE')
